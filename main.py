...
#             }
#         })
#         # Define o cookie. O 'max_age' define quanto tempo ele vai durar (ex: 7 dias)
#         response.set_cookie('user_session', email_usuario, max_age=60 * 60 * 24 * 7) 
#         return response
#     else:
#         return jsonify({'mensagem': 'E-mail ou senha inválidos'}), 401
...
from flask import Flask, jsonify, request
from flask_sqlalchemy import SQLAlchemy
from flask_cors import CORS # Importe esta biblioteca!
...
# Adicione esta linha para habilitar o CORS com credenciais
CORS(app, supports_credentials=True, origins='https://<SEU_USUARIO_GITHUB>.github.io')

db = SQLAlchemy(app)

# Não precisa mais do after_request, a biblioteca CORS faz isso
# @app.after_request
# def after_request(response):
#   response.headers.add('Access-Control-Allow-Origin', '*')
#   response.headers.add('Access-Control-Allow-Headers', 'Content-Type,Authorization')
